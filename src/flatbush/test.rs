...
static DATA: Lazy<Vec<u32>> = Lazy::new(|| {
    vec![
...
fn skips_sorting_less_than_node_size_number_of_rectangles() {
    ...
    for i in (0..(4 * num_items)).step_by(4) {
        if DATA[i] < root_x_min {
            root_x_min = DATA[i];
        }
        if DATA[i + 1] < root_y_min {
            root_y_min = DATA[i + 1];
        }
        if DATA[i + 2] > root_x_max {
            root_x_max = DATA[i + 2];
        }
        if DATA[i + 3] > root_y_max {
            root_y_max = DATA[i + 3];
        }
    }
...
fn performs_bbox_search() {
    ...
    assert_eq!(
        results.sort(),
        [57, 59, 58, 59, 48, 53, 52, 56, 40, 42, 43, 43, 43, 41, 47, 43].sort()
    );
}
...
fn performs_bbox_search_signed_float() {
    ...
    let mut expected = vec![
        -43.0, -41.0, -42.0, -41.0, -52.0, -47.0, -48.0, -44.0, -60.0, -58.0, -57.0, -57.0, -57.0,
        -59.0, -53.0, -57.0,
    ];
